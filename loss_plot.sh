...
CUDA_VISIBLE_DEVICES=$GPU  python3 plot_loss_surface.py   $DATA_PATH --init-model  --normalize-bert --surf-file "$OUTPUT_PATH/ckpt_best_loss_41x41.h5" \
    ...
    --adam-betas '(0.9, 0.98)' --save-dir $OUTPUT_PATH --restore-file $OUTPUT_PATH/checkpoint_best.pt --reset-optimizer \
    --no-progress-bar --log-interval 100 \
    --ddp-backend=no_c10d 2>&1 | tee -a $OUTPUT_PATH/train_log_loss_best.txt
# plot loss surface around init point 
CUDA_VISIBLE_DEVICES=$GPU  python3 plot_loss_surface.py   $DATA_PATH --init-model  --normalize-bert --surf-file "$OUTPUT_PATH/ckpt_init_loss_41x41.h5" \
    ...
    --no-progress-bar --log-interval 100 \
    --ddp-backend=no_c10d 2>&1 | tee -a $OUTPUT_PATH/train_log_loss_init.txt 
